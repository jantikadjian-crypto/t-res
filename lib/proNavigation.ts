...
import { BookOpen, ChartColumn, CreditCard, FolderOpen, Inbox, LayoutDashboard, Settings, ShieldCheck, Users, type LucideIcon } from "lucide-react";
...
import { proLibraryEntry } from "@/lib/proLibrary";
...
export const proNav: ProNavItem[] = [
  ...
  { href: "/pro/library", label: "Library", icon: BookOpen },
...
function segmentLabel(sectionHref: string, segment: string): string {
  ...
  if (sectionHref === "/pro/library") return proLibraryEntry(segment)?.name ?? "Entry";
