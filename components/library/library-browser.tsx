...
import { libraryEntries, libraryFiles, type LibraryKind } from "@/lib/library";
import { cn } from "@/lib/utils";

const KINDS: ("All" | LibraryKind)[] = ["All", "Form", "Notice", "Publication", "Term"];
const KIND_LABEL: Record<"All" | LibraryKind, string> = {
  All: "Everything",
  Form: "Forms",
  Notice: "Notices",
...
export const kindStyle: Record<LibraryKind, string> = {
  ...
  Publication: "border-teal-200 bg-teal-50 text-teal-700",
  Term: "border-purple-200 bg-purple-50 text-purple-700",
};

const hasFiles = (slug: string) => (libraryFiles[slug]?.length ?? 0) > 0;
...
export function LibraryBrowser() {
  ...
  const visible = sorted
    ...
    .filter((e) => !onlyFiles || hasFiles(e.slug))
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div role="group" aria-label="Show" className="grid grid-cols-3 gap-1 rounded-xl bg-muted p-1 sm:grid-cols-5">
            {KINDS.map((k) => (
              <button
                key={k}
                type="button"
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-2">
            <label htmlFor="library-files" className="flex items-center gap-2 text-sm whitespace-nowrap">
              <input
                id="library-files"
                type="checkbox"
                checked={onlyFiles}
                onChange={(e) => setOnlyFiles(e.target.checked)}
      ...
      {visible.length === 0 ? (
        <div className="space-y-3 rounded-xl border border-dashed bg-card p-10 text-center">
          <p className="text-sm text-muted-foreground">Nothing in the Library matches{q ? <> &ldquo;{query}&rdquo;</> : " these filters"}.</p>
            ...
            <li key={e.slug}>
              ...
              >
                <span className="flex flex-wrap items-center gap-2">
                  ...
                  {hasFiles(e.slug) && (
                    <span className="inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-xs font-medium text-muted-foreground">
